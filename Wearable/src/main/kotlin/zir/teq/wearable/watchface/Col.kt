...
data class Col(val name: String, val darkId: Int, val lightId: Int) {
    companion object {
        ...
        fun createPaint(ctx: Context, type: Config.PaintType, col: Col, stroke: Stroke): Paint {
            val paint = when (type) {
                Config.PaintType.HAND -> Col.prepareLinePaint(ctx, col.lightId)
                Config.PaintType.HAND_AMB -> Col.prepareLinePaint(ctx, col.darkId)
                Config.PaintType.SHAPE -> Col.prepareShapePaint(ctx, col.lightId)
                Config.PaintType.SHAPE_AMB -> Col.prepareShapePaint(ctx, col.darkId)
                Config.PaintType.CIRCLE -> Col.prepareCirclePaint(ctx, col.darkId)
                Config.PaintType.CIRCLE_AMB -> Col.prepareCirclePaint(ctx, col.lightId)
                Config.PaintType.TEXT -> Col.prepareTextPaint(ctx, R.color.text)
                Config.PaintType.POINT -> Col.preparePointPaint(ctx, R.color.points)
                else -> {
